...
export default function BookFilter({ activeMethod, onSort }: BookFilterProps) {
  ...
  const SortButton = ({ method, children }: { method: 'title' | 'author' | 'rating', children: React.ReactNode }) => (
    <button
      ...
      className={`bg-transparent px-4 py-1 text-black text-sm font-medium rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-white focus:ring-gray-500 ${activeMethod === method
  ...
  return (
    ...
    <div className="bg-white/20 backdrop-blur-md flex items-center justify-center space-x-2 my-8 p-1 rounded-full border border-gray-200 shadow-sm max-w-sm mx-auto">
